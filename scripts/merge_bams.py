...
def main(args):
    ...
    fm.run_cmd("samtools merge -f - %s | samtools reheader -i %s - | samtools addreplacerg - -r 'ID:%s\\tSM:%s\\tPL:Illumina' -o %s" % (
        " ".join(individual_bams), tmp_file, new_id, new_id, new_bamfile)
    )
