...
def main_map(args):
	...
	fm.run_cmd("rm %(prefix)s_1P %(prefix)s_2P %(prefix)s_1U %(prefix)s_2U")
	if args.bqsr_vcf:
		fm.run_cmd("gatk BaseRecalibrator -R %(ref)s -I %(prefix)s.bam --known-sites %(bqsr_vcf)s -O %(prefix)s.recal_data.table")
		fm.run_cmd("gatk ApplyBQSR -R %(ref)s -I %(prefix)s.bam --bqsr-recal-file %(prefix)s.recal_data.table -O %(prefix)s.bqsr.bam")
		fm.run_cmd("samtools index -@ %(threads)s %(prefix)s.bqsr.bam")
		fm.run_cmd("rm %(prefix)s.bam")
